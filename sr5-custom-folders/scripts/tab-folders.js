 ...
 *
 * Scope keys:
 *   - inventory sections:    `inv:<inventoryName>:<itemType>`
 *   - non-inventory sections: `type:<itemType>`
 *
...
function getHeaderType(header) {
    ...
    return add?.dataset?.itemType ?? null;
}
...
function renderFoldersForSection(app, header, type, scope, itemEls) {
    ...
    // Make every item draggable wrt our system: tag the actual draggable
    // child (`.list-item.draggable`) so dragstart bubbles correctly.
    itemEls.forEach((el) => {
        const dragHandle = el.querySelector(".list-item.draggable") ?? el;
...
function buildFolderWrapper(app, type, scope, folder, items) {
    ...
    head.innerHTML = `
        <span class="sr5cf-folder-toggle">
            <i class="fas fa-${folder.collapsed ? "chevron-right" : "chevron-down"}"></i>
        </span>
        <i class="fas fa-folder"></i>
        <span class="sr5cf-folder-name"></span>
        <span class="sr5cf-folder-count">(${items.length})</span>
        <span class="sr5cf-folder-actions">
            <a data-action="sr5cf-rename" data-tooltip="${L("RenameFolder")}"><i class="fas fa-pen-to-square"></i></a>
            <a data-action="sr5cf-delete" data-tooltip="${L("DeleteFolder")}"><i class="fas fa-trash"></i></a>
        </span>
    `;
 ...
 * core payload intact. The element bound here is the inner `.list-item.draggable`.
 */
...
function attachDropHandlers(app, el, scope, folderId /* string|null */) {
    ...
    el.addEventListener("drop", async (ev) => {
        ...
        // Scope is like "type:weapon" or "inv:<inv>:weapon".
